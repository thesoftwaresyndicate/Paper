...
###############################################################################
"""
The :mod:`Paper.core.common.applocation` module provides an utility for Paper receiving the data path etc.
"""
import logging
import os
import sys

from paper.core.common import Settings, is_win, is_macosx
...
import paper
from paper.core.common import check_directory_exists, get_frozen_path
...
class AppLocation(object):
    ...
    def get_directory(dir_type=AppDir):
        ...
        if dir_type == AppLocation.AppDir:
            return get_frozen_path(os.path.abspath(os.path.dirname(sys.argv[0])), os.path.dirname(paper.__file__))
        elif dir_type == AppLocation.PluginsDir:
            app_path = os.path.abspath(os.path.dirname(sys.argv[0]))
            return get_frozen_path(os.path.join(app_path, 'plugins'),
                                   os.path.join(os.path.dirname(paper.__file__), 'plugins'))
    ...
    def get_data_path():
        """
        Return the path Paper stores all its data under.
        """
...
def _get_os_dir_path(dir_type):
    ...
    if dir_type == AppLocation.LanguageDir:
        directory = os.path.abspath(os.path.join(os.path.dirname(paper.__file__), '..', 'resources'))
        if os.path.exists(directory):
            return directory
    if is_win():
        if dir_type == AppLocation.DataDir:
            return os.path.join(str(os.getenv('APPDATA')), 'Paper', 'data')
        elif dir_type == AppLocation.LanguageDir:
            return os.path.dirname(paper.__file__)
        return os.path.join(str(os.getenv('APPDATA')), 'Paper')
    elif is_macosx():
        if dir_type == AppLocation.DataDir:
            return os.path.join(str(os.getenv('HOME')), 'Library', 'Application Support', 'Paper', 'Data')
        elif dir_type == AppLocation.LanguageDir:
            return os.path.dirname(paper.__file__)
        return os.path.join(str(os.getenv('HOME')), 'Library', 'Application Support', 'Paper')
    else:
        if dir_type == AppLocation.LanguageDir:
            for prefix in ['/usr/local', '/usr']:
                directory = os.path.join(prefix, 'share', 'Paper')
                if os.path.exists(directory):
                    return directory
            return os.path.join('/usr', 'share', 'Paper')
        if XDG_BASE_AVAILABLE:
            if dir_type == AppLocation.DataDir:
                return os.path.join(str(BaseDirectory.xdg_data_home), 'Paper')
            elif dir_type == AppLocation.CacheDir:
                return os.path.join(str(BaseDirectory.xdg_cache_home), 'Paper')
        if dir_type == AppLocation.DataDir:
            return os.path.join(str(os.getenv('HOME')), '.Paper', 'data')
        return os.path.join(str(os.getenv('HOME')), '.Paper')
